...
func TestClientRequestLogging(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "WithLogger",
			configFns: func(w io.Writer) []ConfigFn { return []ConfigFn{WithLogger(getTestLogFn(w))} },
		},
		{
			name: "WithLoggingTransport",
			configFns: func(w io.Writer) []ConfigFn {
				return []ConfigFn{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			c := New("username", "password", append(test.configFns(&output), WithBaseURL(srv.URL))...)
...
{"time":"2 Minutes to Midnight","level":"DEBUG","msg":"Sending request to 127.0.0.1","url":"http://server/1.3/test","method":"POST","headers":{"Accept":["application/json"],"Authorization":["Basic [REDACTED]"],"Content-Type":["application/json"],"User-Agent":["upcloud-go-api/%s"]},"body":"{\n  \"name\": \"test\"\n}"}
