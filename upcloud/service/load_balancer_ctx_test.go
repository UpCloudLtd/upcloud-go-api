...
func TestLoadBalancerContext(t *testing.T) {
	...
	recordWithContext(t, "loadbalancer", func(ctx context.Context, t *testing.T, rec *recorder.Recorder, svcContext *ServiceContext) {
		// Create Load Balancer
		lb, err := createLoadBalancerAndNetwork(ctx, svcContext, "fi-hel1", "172.16.1.0/24")
		require.NoError(t, err)
		t.Cleanup(func() {
			err := cleanupLoadBalancer(ctx, rec, svcContext, lb)
		...
		lb, err = svcContext.ModifyLoadBalancer(ctx, &request.ModifyLoadBalancerRequest{
			UUID: lb.UUID,
			Name: newName,
		})
		require.NoError(t, err)
		...
		lb, err = svcContext.GetLoadBalancer(ctx, &request.GetLoadBalancerRequest{
			UUID: lb.UUID,
		})
		require.NoError(t, err)
		assert.Equal(t, newName, lb.Name)
		...
		lbs, err := svcContext.GetLoadBalancers(ctx, &request.GetLoadBalancersRequest{})
		require.NoError(t, err)
...
func TestLoadBalancerBackendContext(t *testing.T) {
	...
	recordWithContext(t, "loadbalancerbackend", func(ctx context.Context, t *testing.T, rec *recorder.Recorder, svcContext *ServiceContext) {
		lb, err := createLoadBalancerAndNetwork(ctx, svcContext, "fi-hel2", "172.16.2.0/24")
		require.NoError(t, err)
		t.Cleanup(func() {
			err := cleanupLoadBalancer(ctx, rec, svcContext, lb)
			assert.NoError(t, err)
		...
		newName := "updatedName"
		backend, err = svcContext.ModifyLoadBalancerBackend(ctx, &request.ModifyLoadBalancerBackendRequest{
			ServiceUUID: lb.UUID,
			Name:        backend.Name,
		...
		backend, err = svcContext.GetLoadBalancerBackend(ctx, &request.GetLoadBalancerBackendRequest{
			ServiceUUID: lb.UUID,
			Name:        newName,
		})
		require.NoError(t, err)
		...
		backends, err := svcContext.GetLoadBalancerBackends(ctx, &request.GetLoadBalancerBackendsRequest{
			ServiceUUID: lb.UUID,
		})
		require.NoError(t, err)
		...
		err = svcContext.DeleteLoadBalancerBackend(ctx, &request.DeleteLoadBalancerBackendRequest{
			ServiceUUID: lb.UUID,
			Name:        backend.Name,
		})
		require.NoError(t, err)
...
func TestLoadBalancerBackendMemberContext(t *testing.T) {
	...
	recordWithContext(t, "loadbalancerbackendmember", func(ctx context.Context, t *testing.T, rec *recorder.Recorder, svcContext *ServiceContext) {
		lb, err := createLoadBalancerAndNetwork(ctx, svcContext, "nl-ams1", "172.16.3.0/24")
		require.NoError(t, err)
		t.Cleanup(func() {
			err := cleanupLoadBalancer(ctx, rec, svcContext, lb)
			assert.NoError(t, err)
		...
		member, err := svcContext.CreateLoadBalancerBackendMember(ctx, &request.CreateLoadBalancerBackendMemberRequest{
			ServiceUUID: lb.UUID,
		...
		newWeight := 50
		newMaxSessions := 321
		member, err = svcContext.ModifyLoadBalancerBackendMember(ctx, &request.ModifyLoadBalancerBackendMemberRequest{
			ServiceUUID: lb.UUID,
			BackendName: backend.Name,
			Name:        member.Name,
		...
		member, err = svcContext.GetLoadBalancerBackendMember(ctx, &request.GetLoadBalancerBackendMemberRequest{
			ServiceUUID: lb.UUID,
			BackendName: backend.Name,
			Name:        newName,
		})
		...
		members, err := svcContext.GetLoadBalancerBackendMembers(ctx, &request.GetLoadBalancerBackendMembersRequest{
			ServiceUUID: lb.UUID,
			BackendName: backend.Name,
		})
		...
		err = svcContext.DeleteLoadBalancerBackendMember(ctx, &request.DeleteLoadBalancerBackendMemberRequest{
			ServiceUUID: lb.UUID,
			BackendName: backend.Name,
			Name:        member.Name,
		})
...
func TestLoadBalancerResolverContext(t *testing.T) {
	...
	recordWithContext(t, "loadbalancerresolver", func(ctx context.Context, t *testing.T, rec *recorder.Recorder, svcContext *ServiceContext) {
		lb, err := createLoadBalancerAndNetwork(ctx, svcContext, "pl-waw1", "10.0.0.0/24")
		require.NoError(t, err)
		t.Cleanup(func() {
			err := cleanupLoadBalancer(ctx, rec, svcContext, lb)
			assert.NoError(t, err)
		...
		resolver, err := svcContext.CreateLoadBalancerResolver(ctx, &request.CreateLoadBalancerResolverRequest{
			ServiceUUID: lb.UUID,
		...
		resolver, err = svcContext.ModifyLoadBalancerResolver(ctx, &request.ModifyLoadBalancerResolverRequest{
			ServiceUUID: lb.UUID,
			Name:        resolver.Name,
			Resolver: request.LoadBalancerResolver{
				Name:        newName,
		...
		resolver, err = svcContext.GetLoadBalancerResolver(ctx, &request.GetLoadBalancerResolverRequest{
			ServiceUUID: lb.UUID,
			Name:        resolver.Name,
		})
		...
		t.Log("Get resolvers")
		resolvers, err := svcContext.GetLoadBalancerResolvers(ctx, &request.GetLoadBalancerResolversRequest{
			ServiceUUID: lb.UUID,
		})
		require.NoError(t, err)
		...
		err = svcContext.DeleteLoadBalancerResolver(ctx, &request.DeleteLoadBalancerResolverRequest{
			ServiceUUID: lb.UUID,
			Name:        resolver.Name,
		})
		require.NoError(t, err)
...
func TestGetLoadBalancerPlansContext(t *testing.T) {
	recordWithContext(t, "getloadbalancerplans", func(ctx context.Context, t *testing.T, rec *recorder.Recorder, svcContext *ServiceContext) {
		plans, err := svcContext.GetLoadBalancerPlans(ctx, &request.GetLoadBalancerPlansRequest{})
		require.NoError(t, err)
		assert.GreaterOrEqual(t, len(plans), 2)
		plans, err = svcContext.GetLoadBalancerPlans(ctx, &request.GetLoadBalancerPlansRequest{Page: &request.Page{
			Size:   1,
			Number: 0,
		}})
		assert.NoError(t, err)
...
func TestLoadBalancerFrontendContext(t *testing.T) {
	...
	recordWithContext(t, "loadbalancerfrontend", func(ctx context.Context, t *testing.T, rec *recorder.Recorder, svcContext *ServiceContext) {
		lb, err := createLoadBalancerAndNetwork(ctx, svcContext, "de-fra1", "10.0.0.1/24")
		require.NoError(t, err)
		t.Cleanup(func() {
			err := cleanupLoadBalancer(ctx, rec, svcContext, lb)
			assert.NoError(t, err)
		...
		fe, err = svcContext.GetLoadBalancerFrontend(ctx, &request.GetLoadBalancerFrontendRequest{
			ServiceUUID: lb.UUID,
			Name:        fe.Name,
		})
		require.NoError(t, err)
		...
		assert.NoError(t,
			svcContext.DeleteLoadBalancerFrontend(ctx, &request.DeleteLoadBalancerFrontendRequest{
				ServiceUUID: lb.UUID,
				Name:        fe.Name,
			}))
	})
...
func TestLoadBalancerFrontendRuleContext(t *testing.T) {
	t.Parallel()
	const zone = "fi-hel2"
	recordWithContext(t, "loadbalancerfrontendrule", func(ctx context.Context, t *testing.T, rec *recorder.Recorder, svcContext *ServiceContext) {
		net, err := createLoadBalancerAndPrivateNetwork(ctx, svcContext, zone, "10.0.1.1/24")
		require.NoError(t, err)
		...
		rule, err = svcContext.ModifyLoadBalancerFrontendRule(ctx, &request.ModifyLoadBalancerFrontendRuleRequest{
			ServiceUUID:  lb.UUID,
			FrontendName: lb.Frontends[0].Name,
			Name:         rule.Name,
		...
		rule, err = svcContext.ReplaceLoadBalancerFrontendRule(ctx, &request.ReplaceLoadBalancerFrontendRuleRequest{
			ServiceUUID:  lb.UUID,
			FrontendName: lb.Frontends[0].Name,
			Name:         rule.Name,
			Rule: request.LoadBalancerFrontendRule{
		...
		rules, err := svcContext.GetLoadBalancerFrontendRules(ctx, &request.GetLoadBalancerFrontendRulesRequest{
			ServiceUUID:  lb.UUID,
			FrontendName: lb.Frontends[0].Name,
		})
		require.NoError(t, err)
		...
		rule, err = svcContext.GetLoadBalancerFrontendRule(ctx, &request.GetLoadBalancerFrontendRuleRequest{
			ServiceUUID:  lb.UUID,
			FrontendName: lb.Frontends[0].Name,
			Name:         "rule-1",
		})
		require.NoError(t, err)
		require.NoError(t,
			svcContext.DeleteLoadBalancerFrontendRule(ctx, &request.DeleteLoadBalancerFrontendRuleRequest{
				ServiceUUID:  lb.UUID,
				FrontendName: lb.Frontends[0].Name,
				Name:         rule.Name,
			}))
...
func TestLoadBalancerCerticateBundlesAndFrontendTLSConfigsContext(t *testing.T) {
	...
	recordWithContext(t, "loadbalancercerticatebundlesandfrontendtlsconfigs", func(ctx context.Context, t *testing.T, rec *recorder.Recorder, svcContext *ServiceContext) {
		net, err := createLoadBalancerAndPrivateNetwork(ctx, svcContext, "fi-hel1", "10.0.1.1/24")
		require.NoError(t, err)
		feName := "fe-1"
		lb, err := svcContext.CreateLoadBalancer(ctx, &request.CreateLoadBalancerRequest{
		...
		dc, err := svcContext.CreateLoadBalancerCertificateBundle(ctx, &request.CreateLoadBalancerCertificateBundleRequest{
		...
		mc, err = svcContext.ModifyLoadBalancerCertificateBundle(ctx, &request.ModifyLoadBalancerCertificateBundleRequest{
			UUID: mc.UUID,
		...
		dc, err = svcContext.ModifyLoadBalancerCertificateBundle(ctx, &request.ModifyLoadBalancerCertificateBundleRequest{
			UUID: dc.UUID,
		...
		certs, err := svcContext.GetLoadBalancerCertificateBundles(ctx, &request.GetLoadBalancerCertificateBundlesRequest{})
		assert.NoError(t, err)
		...
		cert, err := svcContext.GetLoadBalancerCertificateBundle(ctx, &request.GetLoadBalancerCertificateBundleRequest{UUID: dc.UUID})
		assert.NoError(t, err)
		...
		tls, err := svcContext.CreateLoadBalancerFrontendTLSConfig(ctx, &request.CreateLoadBalancerFrontendTLSConfigRequest{
			ServiceUUID:  lb.UUID,
			FrontendName: feName,
		...
		tls, err = svcContext.ModifyLoadBalancerFrontendTLSConfig(ctx, &request.ModifyLoadBalancerFrontendTLSConfigRequest{
			ServiceUUID:  lb.UUID,
			FrontendName: feName,
			Name:         tls.Name,
			Config: request.LoadBalancerFrontendTLSConfig{
		...
		configs, err := svcContext.GetLoadBalancerFrontendTLSConfigs(ctx, &request.GetLoadBalancerFrontendTLSConfigsRequest{
			ServiceUUID:  lb.UUID,
			FrontendName: feName,
		})
		assert.NoError(t, err)
		...
		tls, err = svcContext.GetLoadBalancerFrontendTLSConfig(ctx, &request.GetLoadBalancerFrontendTLSConfigRequest{
			ServiceUUID:  lb.UUID,
			FrontendName: feName,
			Name:         dc.Name,
		})
		...
		assert.NoError(t, svcContext.DeleteLoadBalancerFrontendTLSConfig(ctx, &request.DeleteLoadBalancerFrontendTLSConfigRequest{
			ServiceUUID:  lb.UUID,
			FrontendName: feName,
			Name:         tls.Name,
		}))
		t.Logf("Deleted TLS config %s", tls.Name)
		assert.NoError(t, svcContext.DeleteLoadBalancerCertificateBundle(ctx, &request.DeleteLoadBalancerCertificateBundleRequest{UUID: mc.UUID}))
		t.Logf("Deleted certificate bundle %s", mc.Name)
		assert.NoError(t, svcContext.DeleteLoadBalancerCertificateBundle(ctx, &request.DeleteLoadBalancerCertificateBundleRequest{UUID: dc.UUID}))
...
func TestLoadBalancerPageContext(t *testing.T) {
	...
	const zone = "fi-hel2"
	recordWithContext(t, "loadbalancerpage", func(ctx context.Context, t *testing.T, rec *recorder.Recorder, svcContext *ServiceContext) {
		net, err := createLoadBalancerAndPrivateNetwork(ctx, svcContext, zone, "172.16.0.0/24")
		require.NoError(t, err)
		lbs := make([]*upcloud.LoadBalancer, 0)
		for i := 0; i < 5; i++ {
			lb, err := createLoadBalancer(ctx, svcContext, net.UUID, zone)
			require.NoError(t, err)
			lbs = append(lbs, lb)
		...
		list, err := svcContext.GetLoadBalancers(ctx, &request.GetLoadBalancersRequest{})
		...
		// test custom page
		list, err = svcContext.GetLoadBalancers(ctx, &request.GetLoadBalancersRequest{Page: &request.Page{
			Size:   2,
			Number: 0,
		}})
		assert.NoError(t, err)
		...
		for _, lb := range lbs {
			if err := svcContext.DeleteLoadBalancer(ctx, &request.DeleteLoadBalancerRequest{UUID: lb.UUID}); err != nil {
				t.Logf("an error occurred when deleting LB '%s': %v", lb.Name, err)
				continue
			}
		}
		for _, lb := range lbs {
			if err := waitForLoadBalancerToShutdown(ctx, rec, svcContext, lb); err != nil {
				t.Log(err)
				continue
			}
		}
		if err := svcContext.DeleteNetwork(ctx, &request.DeleteNetworkRequest{UUID: net.UUID}); err != nil {
			t.Log(err)
		}
...
func TestLoadBalancerNetworkContext(t *testing.T) {
	...
	recordWithContext(t, "loadbalancernetworks", func(ctx context.Context, t *testing.T, rec *recorder.Recorder, svcContext *ServiceContext) {
		const zone string = "pl-waw1"
		net, err := createLoadBalancerPrivateNetwork(ctx, svcContext, zone, "192.168.55.0/24")
		require.NoError(t, err)
...
func waitLoadBalancerOnline(ctx context.Context, t *testing.T, rec *recorder.Recorder, svcContext *ServiceContext, UUID string) error {
	t.Helper()
	if rec.Mode() == recorder.ModeRecording {
		...
		for s := time.Now(); time.Since(s) < time.Minute*20; {
			lb, err := svcContext.GetLoadBalancer(ctx, &request.GetLoadBalancerRequest{UUID: UUID})
			if err != nil {
				return err
			}
...
func createLoadBalancerBackendContext(ctx context.Context, svcContext *ServiceContext, lbUUID string) (*upcloud.LoadBalancerBackend, error) {
	...
	return svcContext.CreateLoadBalancerBackend(ctx, &req)
}

func cleanupLoadBalancer(ctx context.Context, rec *recorder.Recorder, svcContext *ServiceContext, lb *upcloud.LoadBalancer) error {
	if rec.Mode() != recorder.ModeRecording {
		return nil
	}
	rec.AddPassthrough(func(h *http.Request) bool {
	...
	err := deleteLoadBalancer(ctx, svcContext, lb)
	rec.Passthroughs = nil
	return err
}

func createLoadBalancer(ctx context.Context, svcContext *ServiceContext, networkUUID, zone string) (*upcloud.LoadBalancer, error) {
	...
	loadBalancerDetails, err := svcContext.CreateLoadBalancer(ctx, &createLoadBalancerRequest)
	if err != nil {
		return nil, err
	}
...
func createLoadBalancerAndNetwork(ctx context.Context, svcContext *ServiceContext, zone, addr string) (*upcloud.LoadBalancer, error) {
	n, err := createLoadBalancerAndPrivateNetwork(ctx, svcContext, zone, addr)
	if err != nil {
		return nil, err
	}
	return createLoadBalancer(ctx, svcContext, n.UUID, zone)
}

func createLoadBalancerAndPrivateNetwork(ctx context.Context, svcContext *ServiceContext, zone, addr string) (*upcloud.Network, error) {
	return svcContext.CreateNetwork(ctx, &request.CreateNetworkRequest{
...
func waitForLoadBalancerToShutdown(ctx context.Context, rec *recorder.Recorder, svcContext *ServiceContext, lb *upcloud.LoadBalancer) error {
	if rec.Mode() != recorder.ModeRecording {
		return nil
	}
	...
	for i := 0; i <= maxRetries; i++ {
		_, err := svcContext.GetLoadBalancer(ctx, &request.GetLoadBalancerRequest{UUID: lb.UUID})
		if err != nil {
...
func waitLoadBalancerToShutdown(ctx context.Context, svcContext *ServiceContext, lb *upcloud.LoadBalancer) error {
	const maxRetries int = 100
	// wait delete request
	for i := 0; i <= maxRetries; i++ {
		_, err := svcContext.GetLoadBalancer(ctx, &request.GetLoadBalancerRequest{UUID: lb.UUID})
...
func deleteLoadBalancer(ctx context.Context, svcContext *ServiceContext, lb *upcloud.LoadBalancer) error {
	if err := svcContext.DeleteLoadBalancer(ctx, &request.DeleteLoadBalancerRequest{UUID: lb.UUID}); err != nil {
		return err
	}

	if err := waitLoadBalancerToShutdown(ctx, svcContext, lb); err != nil {
		return fmt.Errorf("unable to shutdown LB '%s' (%s) (check dangling networks)", lb.UUID, lb.Name)
	}

	var errs []error
	if lb.NetworkUUID != "" {
		if err := svcContext.DeleteNetwork(ctx, &request.DeleteNetworkRequest{UUID: lb.NetworkUUID}); err != nil {
	...
	if len(lb.Networks) > 0 {
		for _, n := range lb.Networks {
			if n.Type == upcloud.LoadBalancerNetworkTypePrivate && n.UUID != "" {
				if err := svcContext.DeleteNetwork(ctx, &request.DeleteNetworkRequest{UUID: n.UUID}); err != nil {
					errs = append(errs, err)
				}
			}
		}
...
func createLoadBalancerPrivateNetwork(ctx context.Context, svcContext *ServiceContext, zone, addr string) (*upcloud.Network, error) {
	return svcContext.CreateNetwork(ctx, &request.CreateNetworkRequest{
		Name: fmt.Sprintf("go-test-lb-%d", time.Now().Unix()),
		Zone: zone,
		IPNetworks: []upcloud.IPNetwork{
